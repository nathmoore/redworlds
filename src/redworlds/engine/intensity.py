...
- **2011 → 2026 is a matter of record.** We know what happened. Solar and wind costs
...
- **2026 → 2050 is a scenario.** Nobody knows. SSP2 is the defensible choice and is what the
  game's baseline assumes, but it is a choice.

A note on vintage, since the pivot is the year we are in: complete annual energy and
emissions statistics lag by a year or more, so "observed to 2026" rests on series that run to
2024 or 2025. That is a known and small imprecision — a year of a 2%/yr trend — and it is the
same imprecision the money conversion already carries, because the 2026 CPI figure is a
projection for a year that is not over either. Stated once here rather than hedged everywhere.
...
# The present day: where observation stops and scenario begins.
#
# Deliberately the same year as engine/currency.py's DISPLAY_YEAR. What a euro emits and what
# a euro buys are different questions and need not share a year, but having them differ buys
# nothing and costs a reader one more thing to hold in their head — so they are kept in step,
# and tests/engine/test_intensity.py asserts it so they cannot drift apart again.
PRESENT_YEAR: int = 2026

# The game's "now", and the year every tape is scored from.
TARGET_YEAR: int = 2050

# Stage one, 2011 → 2026. OBSERVED.
#
# Global CO2 intensity of GDP was 27% below its 2010 level by 2025 (Enerdata) — so 0.73 over
# those fifteen years, which is -2.08%/yr compounded and consistent with the -2.2%/yr Enerdata
# reports for 2010-2019 directly. 2011 → 2026 is also fifteen years, so at a steady rate the
# published figure carries straight over and this stage is the reported number rather than
# something extrapolated from it.
#
...
OBSERVED_2011_TO_2026: float = 0.73
"""Sourced and checkable. See docs/references.md."""

# Stage two, 2026 → 2050. SCENARIO.
#
# Continues the observed 2.08%/yr decline across the twenty-four years to 2050, which is
...
SCENARIO_2026_TO_2050: float = 0.60
...
def intensity_scalar(year: int = TARGET_YEAR) -> float:
    ...
    Args:
        year: The year to correct to. ``BASE_YEAR`` (no correction), ``PRESENT_YEAR`` (the
    ...
    if year == PRESENT_YEAR:
        return OBSERVED_2011_TO_2026
    if year == TARGET_YEAR:
        return OBSERVED_2011_TO_2026 * SCENARIO_2026_TO_2050
    raise ValueError(
        f"intensity_scalar answers for {BASE_YEAR}, {PRESENT_YEAR} and {TARGET_YEAR} while the "
